<article class="flex flex-col gap-4 p-4 bg-white-medium rounded-2xl">
    <header>
        <h3 class="text-h3">
            {{ $event->title }}
        </h3>
        ...
        @endif
        <span>- {{ $event->participants->count() }} @if($event->participants->count()>1) participants @else participant @endif </span>
    </header>
    ...
    @else
        <section class="flex flex-col gap-2 p-4 rounded-lg bg-green-light text-green-dark">
            <h4 class="text-h4">
                Vous participez
